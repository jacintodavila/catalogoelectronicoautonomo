...
%
% a GUI interface for swish-e, the indexing system
% originally based on code from the library of examples in www.swi-prolog.org
% by Yaritza Vargas <vargas.yaritza@gmail.com> and Jacinto Dávila <jacinto@ula.ve>
...
buscar(_) :->
    ...
    send(@browser, open_message, message(@prolog, invoca, firefox, @arg1?key)).
...
invoca(Programa, Argumento) :-
    concat(Programa, ' ', P),
    concat(P, Argumento, Command), 
    shell(Command, _).

/* Processing information from the indexer */
search_index(Query, Reporte) :- 
	retractall(salida(_)),
	call_unix_temp('swish-e', [' -f ./indices/index.swish-e -w ', Query], Output), 
...
concatena(Lista, Comando) :-
	concatenar(Lista, '', Comando).

concatenar([U], Previo, Comando) :- 
	concat(Previo, U, Comando).

concatenar([U|R], Previo, Comando) :-
	concat(Previo, U, Siguiente),
	concatenar(R, Siguiente, Comando).

test_concatena :-
	concatena(['swish-e',' -w ' ,'Asociación de Profesores del Instituto Pedagógico de Caracas UPEL'], 'swish-e -w Asociación de Profesores del Instituto Pedagógico de Caracas UPEL'). 
...
% qsave_program(cea, [goal=l, autoload=true, stand_alone=true]).

% test

test_s :-  
 	search_index('Asociación de Profesores del Instituto Pedagógico de Caracas UPEL', _A). 

test_search(Desc, Answers) :- 
   search_index(Desc, (_,Answers)).

test_swish1 :- 
   shell('swish-e -f ./indices/index.swish-e -w "Asociación de Profesores del Instituto Pedagógico de Caracas UPEL"', _L).

test_swish3 :- 
   shell('swish-e -f ./indices/index.swish-e -w "Jacinto Dávila"', _L).
